...
export function spriteConcurrencyForIntensity(intensity: "normal" | "high" | "maximum", shardCount: number): number {
  const cap = intensity === "maximum"
    ? 8
    : intensity === "high"
      ? 4
      : 2;
  return Math.max(1, Math.min(shardCount, cap));
}
