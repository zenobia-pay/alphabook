...
export async function cleanupToolStreamWithWorkersAi(
  ...
  const prompt = [
    ...
    "Keep every retained line terse, factual, and easy for a smart non-technical reader to understand.",
    "Describe the actual task intent, search terms, model prompt intent, concrete result, failure, or retry decision.",
    "Do not narrate infrastructure motion such as launching, preparing, heartbeats, file creation, status polling, proxy transport, or model selection.",
    ...
    "Prefer search targets, comparison targets, failure causes, and next actions over implementation detail.",
    "If the batch contains no user-meaningful activity, return an empty normalizedLines array and an empty summary.",
