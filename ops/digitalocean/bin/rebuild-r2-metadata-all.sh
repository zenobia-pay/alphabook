...
MIRROR_CHECKPOINT_PATH="${MIRROR_CHECKPOINT_PATH:-$ALPHABOOK_ROOT/.alphabook/rebuild-metadata-checkpoint.json}"
LOG_PATH="${LOG_PATH:-$ALPHABOOK_ROOT/logs/rebuild-r2-metadata-all.log}"
...
export CANONICAL_REBUILD_METADATA_ONLY=1
...
while true; do
  ...
  if [[ "$processed" == "0" ]]; then
    printf '[%s] rebuilt all canonical D1 metadata rows\n' "$(date -Is)" | tee -a "$LOG_PATH"
    break
  fi
